...
def evaluate_pipelines_performance_averaged(
    ...
    for i in range(OverAllResults.shape[0]):
        ...
        for t in TestSets:
            ...
            avg_predictions = Training_data_predictions.groupby(grouping_column)[
                "predictions"
            ].mean()
            avg_target = Training_data_predictions.groupby(grouping_column)[
                target
            ].mean()

            T_MAEs.append(mean_absolute_error(avg_target, avg_predictions))
            T_NMAEs.append(
                mean_absolute_error(avg_target, avg_predictions)
                / (max(avg_target) - min(avg_target))
            )
...
def pipeline_LOOCV_evaluation(
    ...
    for p_idx, (preprocessing_name, pipeline) in enumerate(
        ...
        for test_sample in file[Sample_ID].unique():
            ...
            testing_target = Testing_data.groupby(Testing_data[Sample_ID])[
                target
            ].mean()
            testing_features = (
                Testing_data.iloc[:, Spectra_Start_Index:]
                .groupby(Testing_data[Sample_ID])
                .mean()
            )
...
def FeatureImportanceEvaluation_Retrain(
    ...
    predictions_SA_long = predictions_df.explode(
    ...
    for col in prediction_columns:
        results_summary_SA[col] = evaluate_predictions(
            Final_Results_5CV_ALL_SA["Groundtruths"], Final_Results_5CV_ALL_SA[col]
        )
...
def feature_block_importance(
    ...
    for start_idx in range(1, num_features - interval_size + 1, step_size):
        ...
        message = (
            f"Evaluating Excluded Features {start_idx} | Elapsed: {elapsed_min:.2f} min"
        )
